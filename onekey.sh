...
let_robot_go(){
	...
	print_command "roslaunch spark_teleop teleop.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}"
	roslaunch spark_teleop teleop.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
}
...
remote_control_robot(){
	...
	echo -e "${Info}" 
	echo && stty erase ^? && read -p "按回车键（Enter）开始：" 
	print_command "roslaunch spark_teleop app_op.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}"
	roslaunch spark_teleop app_op.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
}
...
people_follow(){
	...
	echo -e "${Info}" 
	echo && stty erase ^? && read -p "按回车键（Enter）开始：" 
	print_command "roslaunch spark_follower bringup.launch camera_types:=${CAMERATYPE}"
	roslaunch spark_follower bringup.launch camera_types:=${CAMERATYPE} 
}
...
voice_nav(){
	...
	echo -e "${Info}" 
	echo && stty erase ^? && read -p "按回车键（Enter）开始：" 
	print_command "roslaunch spark_voice ${ASRTYPE} camera_types:=${CAMERATYPE}"
	roslaunch spark_voice ${ASRTYPE} camera_types:=${CAMERATYPE}
...
cal_camera_arm(){
	...
	if [ $ROSVER = "kinetic" ]; then
		...
		print_command "roslaunch spark_carry_object spark_carry_cal_cv3.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}"
	  	roslaunch spark_carry_object spark_carry_cal_cv3.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
	elif [ $ROSVER = "indigo" ]; then
		echo -e "${Info}It is indigo." 
		print_command "roslaunch spark_carry_object spark_carry_cal_cv2.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}"
	  	roslaunch spark_carry_object spark_carry_cal_cv2.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
...
spark_navigation_2d(){
	...
	print_command "roslaunch spark_navigation amcl_demo_lidar_rviz.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}"
	roslaunch spark_navigation amcl_demo_lidar_rviz.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
}
#让SPARK使用深度摄像头进行导航
spark_navigation_3d(){
	echo -e "${Info}" 
	...
	echo && stty erase ^? && read -p "按回车键（Enter）开始：" 
	if [[ "${SLAMTYPE}" == "2d" ]]; then
		print_command "roslaunch spark_navigation amcl_demo_rviz.launch camera_types:=${CAMERATYPE}"
		roslaunch spark_navigation amcl_demo_rviz.launch camera_types:=${CAMERATYPE} 
	else
		print_command "roslaunch spark_rtabmap spark_rtabmap_nav.launch camera_types:=${CAMERATYPE}"	
		roslaunch spark_rtabmap spark_rtabmap_nav.launch camera_types:=${CAMERATYPE} 
	fi	
}
...
spark_DeepLearn(){
	...
	echo && stty erase ^? && read -p "请选择 1 或 2 ：" chnum
 	case "$chnum" in
		1)
		source ${PROJECTPATH}/devel/setup.bash
		print_command "roslaunch tensorflow_object_detector object_detect.launch camera_types:=${CAMERATYPE}"	
		roslaunch tensorflow_object_detector object_detect.launch camera_types:=${CAMERATYPE}		
		;;
		2)
		roslaunch darknet_ros deeplearn_darknet_yoloV3.launch camera_types:=${CAMERATYPE}
		;;
		*)
...
spark_carry_obj(){
	...
	if [ $ROSVER = "kinetic" ]; then
		echo -e "${Info}It is kinetic." 
		print_command "roslaunch spark_carry_object spark_carry_object_only_cv3.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}"
	  	roslaunch spark_carry_object spark_carry_object_only_cv3.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
	elif [ $ROSVER = "indigo" ]; then
		echo -e "${Info}It is indigo." 
		print_command "roslaunch spark_carry_object spark_carry_object_only_cv2.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}"
	  	roslaunch spark_carry_object spark_carry_object_only_cv2.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
	elif [ $ROSVER = "melodic" ]; then
		echo -e "${Info}It is melodic." 
		print_command "roslaunch spark_carry_object spark_carry_object_only_cv3.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}"
	  	roslaunch spark_carry_object spark_carry_object_only_cv3.launch camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
...
spark_build_map_2d(){
	...
	echo -e "${Info}" 
	echo && stty erase ^? && read -p "按回车键（Enter）开始：" 
	print_command "roslaunch spark_slam 2d_slam_teleop.launch slam_methods_tel:=${SLAMTYPE} camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}"
	roslaunch spark_slam 2d_slam_teleop.launch slam_methods_tel:=${SLAMTYPE} camera_types:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
...
spark_carry_game(){
	...
	case "$armnum" in
		1)
		roslaunch move2grasp teleop2grasp.launch camera_types:=${CAMERATYPE}  lidar_type_tel:=${LIDARTYPE}
		;;
		2)
		roslaunch move2grasp move2grasp.launch camera_types:=${CAMERATYPE}  lidar_type_tel:=${LIDARTYPE}
		;;
		*)
...
spark_test_mode(){
	...
	echo && stty erase ^? && read -p "请选择 1 或 2 ：" chnum
 	case "$chnum" in
		1)
		roslaunch spark_test all_run_test_st.launch camera_types:=${CAMERATYPE}	
		;;
		2)
		roslaunch spark_test all_run_test.launch camera_types:=${CAMERATYPE}	
		;;
		*)
		echo -e "${Error} 退出!"	
		;;
...
spark_build_map_3d(){
	...
	if [[ "${SLAMTYPE}" == "rtab_map" ]]; then
		...
		if [[ "${choose}" == "y" ]]; then
			print_command "roslaunch spark_rtabmap spark_rtabmap_teleop.launch camera_types:=${CAMERATYPE}"
                	roslaunch spark_rtabmap spark_rtabmap_teleop.launch camera_types:=${CAMERATYPE}
		else
			return
		fi
        else
        	print_command "roslaunch spark_slam depth_slam_teleop.launch slam_methods_tel:=${SLAMTYPE} camera_types:=${CAMERATYPE}"
		roslaunch spark_slam depth_slam_teleop.launch slam_methods_tel:=${SLAMTYPE} camera_types:=${CAMERATYPE}
	fi
...
calibrate_camera(){
	...
	PROJECTPATH=$(cd `dirname $0`; pwd)
	source ${PROJECTPATH}/devel/setup.bash
	print_command "roslaunch camera_calibrator camera_calibrate.launch calibrate_type:=${CALIBRATETYPE} camera_types:=${CAMERATYPE}"
	roslaunch camera_calibrator camera_calibrate.launch calibrate_type:=${CALIBRATETYPE} camera_types:=${CAMERATYPE}
